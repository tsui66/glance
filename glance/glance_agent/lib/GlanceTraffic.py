...
gather_opts = [
        cfg.StrOpt('access_log_dir',\
                    default = '/var/log/ln_access/access*.log*',
...
class GatherLogData(object):
    ...
    def grab_log(self, log_path, rpcfunc):
        #logs =[]
        ...
        for line in open(log_path, 'r'):
            ...
            try:
                rpcfunc.getTraffic(data = traffic)
            except Exception as e:
                LOG.Exception(str(e))

    def gather_log(self, rpcfunc):
        for file in self.file_list:
            self.grab_log(file, rpcfunc)
