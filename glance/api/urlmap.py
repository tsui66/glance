...
class Accept(object):
    ...
    def __init__(self, value):
        self._content_types = [parse_options_header(v)
                               for v in parse_list_header(value)]
            ...
            for content_mask, params in self._content_types:
                ...
                if quality < best_quality:
                    ...
                    if best_match.count('*') <= content_mask.count('*'):
                        continue
                if self._match_mast(content_mask, content_type):
    ...
    def _match_mast(self, mask, content_type):
        if '*' not in mask:
...
class URLMap(paste.urlmap.URLMap):
    ...
    def _set_scritp_name(self, app, app_url):
    ...
    def _content_type_strategy(self, host, port, environ):
        ...
        if 'version' in params:
            ...
            if app:
                app = self._set_scritp_name(app, app_url)
        return app
    ...
    def __call__(self, environ, start_response):
        ...
        if ":" in host:
            ...
            if environ['wsgi.url_schema'] == 'http':
                port = '80'
            else:
                port = '443'
            ...
            if app:
                environ['app.best_content_type'] = mime_type
                return app(environ, start_response)
